-- Phase 1 hardening:
--   (1) Cross-club referential integrity triggers. Foreign keys guarantee that a
--       referenced row EXISTS, but not that it belongs to the SAME club. Without
--       these checks an owner/treasurer of club A could (via a crafted request)
--       reference a member, plan or cycle from club B. Each trigger below rejects
--       such cross-club references. Triggers are SECURITY DEFINER so the existence
--       check is authoritative and not affected by the caller's RLS visibility.
--   (2) Server-side dues helpers used by Edge Functions and the mobile client:
...
-- ---------------------------------------------------------------------------
-- (2) Server-side dues helpers (owner/treasurer authorized via my_member_role)
-- ---------------------------------------------------------------------------
